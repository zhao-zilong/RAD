...
def gennoise(train_labels, n_classes, n_init, last_clean, batch_size, time_horizon, noise_min, noise_max, noise_type, noise_steps):
    ...
    for i in np.arange(time_horizon):
        ...
        if noise_type[j] == 'constant':
            ...
            noise_per_batch[i] = min(max(int(
                np.random.normal(noise_mean, noise_std) * batch_size),0),batch_size)  # number of noisy samples for each batch
